...
export async function generateText(prompt: string): Promise<string> {
    try {
        // Priority 1: Try Pollinations.ai with retries (free, no API key needed)
        const pollinationsResult = await tryPollinationsText(prompt);
        if (pollinationsResult) {
            return pollinationsResult;
        }

        // Priority 2: Fallback to Gemini only if API key is available
        if (!apiKey) {
            console.error("⚠️ All Pollinations models failed and Gemini API key not available");
            throw new Error("Text generation failed: Pollinations.ai unavailable and no Gemini API key");
        }
